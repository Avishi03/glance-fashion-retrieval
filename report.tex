...
\midrule
\textbf{Precision@5} & \textbf{0.64} & \textbf{0.32} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}\small
  \item \textbf{Query 1 is the weakest (1/5).} Fashionpedia has no \texttt{raincoat}
        category, only coat/jacket/cape, so only one true raincoat surfaces and
        ``raincoat'' otherwise degrades to ``yellow garment'': the tail is yellow tops
        and an orange dress. A vocabulary gap in the corpus, not in the model.
